...
TPL = """<!doctype html><html><head><meta charset="utf-8"><style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  h1 {{ font-size:{size}px; line-height:1.16; font-weight:800; letter-spacing:-.01em;
    max-width:1010px; }}
  ...
  <div><h1>{title}</h1><div class="rule"></div></div>
...
def main():
    ...
    for f in sorted(os.listdir(ARTICLES)):
        ...
        size = 72 if len(title) <= 42 else (60 if len(title) <= 62 else 50)
        page = TPL.format(series=SERIES.get(num, "IDEAS &amp; RESEARCH"),
                          title=html.escape(title), size=size)
